...
def kaks_attribute(line, tempfilename, CDSpair, apps_loc, ):
    if len(CDSpair[0]) == len(CDSpair[1]):
                            ...
                            try:
                                ...
                                return Ka, Ks, KaKs, KaKs_success
                            except:
                                KaKs_success = False
...
def weboutput(unique_file_id, tid, cod, db_version, seqqu, database_n, minm,
    ...
    tempfilename = data_loc + unique_file_id
    ...
    if form.has_key("seqquery"):
        ...
        if (seqquery[:3] == "NM_") or (seqquery[:3] == "XM_") or (seqquery[:3] == "GI:"):
            ...
            details = accessions.getmRNA(seqquery, dbstuff[database_n], codon, mRNAparams[0] * 3, mRNAparams[1] * 3,
                                         mRNAparams[2])
            if details:
...
def strictmode_and_filter_trigger(unique_file_id, strict, filter_org, starting_amino):
    ...
    if strict == 1:
        if filter_org == 'None':
            ...
            match_string = '<br><b> Non-strict mode: %i hits found.</b><br><br>'
            ...
            match_string = '<br><b> Strict mode: %i filtered hits found.</b><br><br>'
            return sql_non_trivial_retrieve, match_string
    else:
        if filter_org == 'None':
...
kakst = []
strictt = []
if form.has_key("kaks"):
    kakst.append(1)
else:
    kakst.append(0)
if form.has_key("strict"):
    strictt.append(1)
else:
    strictt.append(0)
...
for i in range(0, 5):
    if form.has_key("g" + gradnames[i]):
        gradi.append(float(form["g" + gradnames[i]].value) / 100)
    else:
        gradi.append(None)
if form.has_key("seqquery"):
    ...
    if len(seqquery) == 28:
        ...
        if not cursor.rowcount:
            cursor.close()
            dbcon.close()
            weboutput(unid, timeid, form["codon"].value, form["database_version"].value, form["seqquery"].value,
                      form["database"].value, mRNAparams[0],
                      mRNAparams[1], mRNAparams[2], form["heatmapsize"].value, kakst[0],
                      form["window"].value, gradi, refm[0])
            weboutput2(unid, form["filter"].value, strictt[0])
        else:
            result = cursor.fetchone()
            weboutput2(result[1], form["filter"].value, strictt[0])
